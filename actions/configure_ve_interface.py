...
class CreateVe(Action):
    ...
    def run(self, host=None, username=None, password=None, rbridge_id=None,
            ve_vlan=None, ve_ip=None):
        ...
        with pynos.device.Device(conn=conn, auth=auth) as device:
            changes['create_ve'] = self._create_ve(device, rbridge_id=rbridge_id,
                                                   ve_name=ve_vlan, ip_address=ve_ip)
            changes['set_active'] = self._set_ve_active(device, ve_name=ve_vlan,
    ...
    def _set_ve_active(self, device, ve_name, rbridge_id):
        output = device.interface.admin_state(int_type='ve', name=ve_name,
                                              enabled=True, rbridge_id=rbridge_id)
        return output
